...
def get_temperature(json_data):
    # Retrieve the temperature from the json object
    temp_in_celcius = json_data['main']['temp']
    return temp_in_celcius

def get_weather_type(json_data):
    # Retrieve the weather type from the json object
    weather_type = json_data['weather'][0]['description']
    return weather_type

def get_wind_speed(json_data):
    # Retrieve the current wind speed from the json object
...
def get_weather_data(json_data, city):
    # gather all data and set to variables
    ...
    # format a string with all of the variables and information
    weather_details = ''
